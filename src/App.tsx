...
import { FaWhatsapp, FaTrophy, FaTrash, FaSave, FaCog, FaTimes, FaHistory, FaEdit, FaCommentDots, FaPaperPlane } from 'react-icons/fa';
...
export default function AdhikotProAdvanced() {
  ...
  const triggerAnim = (txt) => { 
    ...
    postCommentary(txt); // Auto-commentary on big events
  };

  const postCommentary = (text) => {
    if (!text || !match) return;
    const newComm = [text, ...(match.commentary || [])].slice(0, 5); // Keep last 5
  ...
  const handleScore = (runs, type = 'normal', outType = null) => {
    ...
    if (type === 'normal') {
      ...
      postCommentary(`Wide Ball +${runs} runs`);
    } else if (type === 'nb') {
      data.score += (1 + runs); data.bwr_r += (1 + runs);
      striker.r = (parseInt(striker.r) || 0) + runs; striker.b += 1;
      postCommentary(`No Ball +${runs} runs`);
    }
    ...
    <div style={s.container}>
      ...
      {!match ? (
        <div style={s.setupBox}>
          ...
          <form onSubmit={(e) => {
            ...
            set(ref(db, 'liveMatch'), {
              lg: fd.get('lg'), t1, t2, date: fd.get('date'), maxOv: parseInt(fd.get('max')),
              t1p: fd.get('t1p').split(','), t2p: fd.get('t2p').split(','),
              batTeam: fd.get('batFirst') === 'T1' ? t1 : t2, bowlTeam: fd.get('batFirst') === 'T1' ? t2 : t1,
              s1: {n: 'Striker', r: 0, b: 0}, s2: {n: 'Non-Striker', r: 0, b: 0}, 
            ...
            <select name="batFirst" style={s.input} required>
                <option value="T1">Team A Batting</option>
                <option value="T2">Team B Batting</option>
            </select>
        ...
        <div style={{padding:'10px'}}>
          <div style={s.card}>
            {match.status === 'Finished' ? (
              <div style={s.fullScoreboard}>
                 ...
                 <h2 style={{color:'#facc15'}}>MATCH SUMMARY</h2>
                 <p>{match.batTeam} vs {match.bowlTeam}</p>
              ...
              <>
                <div style={{fontSize:'14px', color: '#facc15', textTransform: 'uppercase'}}><b>{match.batTeam} BATTING</b></div>
          ...
          {/* COMMENTARY BOX */}
          <div style={s.commBox}>
            <div style={{color:'#facc15', fontSize:'12px', marginBottom:'5px'}}><FaCommentDots/> LIVE COMMENTARY</div>
            {match.commentary?.map((c, i) => (
              <div key={i} style={{...s.commItem, opacity: i === 0 ? 1 : 0.5}}>
            ...
            <div style={s.adminGrid}>
              {match.status !== 'Finished' && (
                <>
                  ...
                  <button onClick={()=>setWktModal(true)} style={s.wktBtn}>WKT</button>
                  
                  {/* Commentary Input for Admin */}
                  <div style={{gridColumn:'span 2', display:'flex', gap:'5px'}}>
                    <input value={commInput} onChange={e=>setCommInput(e.target.value)} placeholder="Type commentary..." style={s.commInput} />
                    <button onClick={()=>postCommentary(commInput)} style={s.postBtn}><FaPaperPlane/></button>
                  </div>
                  
                  <button onClick={undoLastAction} style={s.editBtn}><FaEdit/></button>
                  <button onClick={shareToWhatsApp} style={s.waBtn}><FaWhatsapp/></button>
                </>
              )}
              
              <div style={{gridColumn:'span 3', display:'flex', gap:'10px', marginTop:'10px'}}>
                {match.status === 'Innings Break' && (
                  <button onClick={() => update(ref(db, 'liveMatch'), {innings: 2, score: 0, wkts: 0, ov: 0, bl: 0, target: match.score + 1, status: 'Live', batTeam: match.bowlTeam, bowlTeam: match.batTeam, bwr_r:0, bwr_w:0, bwr_o:0, bwr:'Bowler', commentary: ["Second Innings Started!"]})} style={s.saveBtn}>START 2ND INN</button>
                )}
                <button onClick={saveMatchToHistory} style={s.saveBtn}><FaSave/> SAVE</button>
                {match.status === 'Finished' && <button onClick={closeMatch} style={s.delBtn}>CLOSE</button>}
              </div>
            </div>
          )}
        </div>
      ...
      {/* Modals for Bowler/Batsman/Extras remain same */}
      {isAdmin && selModal && (
        <div style={s.overlay} onClick={() => setSelModal(null)}><div style={s.modal} onClick={e => e.stopPropagation()}>
          ...
          {(selModal === 'bwr' ? (match.batTeam === match.t1 ? match.t2p : match.t1p) : (match.batTeam === match.t1 ? match.t1p : match.t2p)).map((p, i) => (
            <div key={i} style={s.pItem} onClick={() => {
                const up = {};
                if(selModal==='s1') up.s1 = {n:p, r:0, b:0};
                if(selModal==='s2') up.s2 = {n:p, r:0, b:0};
...
const s = {
  ...
  commItem: { fontSize:'13px', padding:'5px 0', borderBottom:'1px solid #1e293b', display:'flex', alignItems:'center', gap:'8px' },
  ...
  waBtn: { padding:'15px', background:'#25D366', color:'white', borderRadius:'10px', border:'none' },
  editBtn: { padding:'15px', background:'#3b82f6', color:'white', borderRadius:'10px', border:'none' },
  ...
  saveBtn: { flex:1, padding:'12px', background:'#22c55e', color:'white', borderRadius:'8px', border:'none' },
  delBtn: { flex:1, padding:'12px', background:'#b91c1c', color:'white', borderRadius:'8px', border:'none' },
