...
import { FaWhatsapp, FaSyncAlt, FaUserShield, FaTrophy, FaTrash, FaSave, FaPlay } from 'react-icons/fa';
...
export default function AdhikotProExpert() {
  ...
  if (!isAdmin && !match) {
    return (
      <div style={s.container}><div style={s.authBox}>
  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.header}><div style={s.flexBetween}>
           <div style={s.flex}><div style={s.avatar}>T</div><div><b>Touqeer Iqbal</b><br/><small style={{color:'#ef4444'}}>● LIVE</small></div></div>
           <FaWhatsapp size={24} color="#22c55e" onClick={() => window.open(`https://wa.me/923015800630?text=Match Update: ${match?.score}/${match?.wkts}`)}/>
      </div></div>

      {!match ? (
        <div style={s.setupBox}>
          <h3 style={{color:'#facc15'}}><FaTrophy/> Setup Match</h3>
          <form onSubmit={(e) => {
            e.preventDefault();
            const fd = new FormData(e.target);
            set(ref(db, 'liveMatch'), {
              lg: fd.get('lg'), toss: fd.get('toss'), t1: fd.get('t1'), t2: fd.get('t2'), 
              c1: fd.get('c1'), c2: fd.get('c2'), emp: fd.get('emp'),
              date: fd.get('date'), time: fd.get('time'), maxOv: parseInt(fd.get('max')),
            ...
            <textarea name="t1p" placeholder="Team A Squad" style={s.area}/><textarea name="t2p" placeholder="Team B Squad" style={s.area}/>
