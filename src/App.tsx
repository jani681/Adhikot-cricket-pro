...
export default function AdhikotProProfessional() {
  ...
  const handleScore = (runs, type = 'normal', outType = null) => {
    // 1. STRCIT GUARD: Agar match finish ya break par hai, ya overs poore ho chuke hain, to kuch na karo.
    if (!match || match.status === 'Finished' || match.status === 'Innings Break') return;
    if (match.ov >= match.maxOv) return; 
    ...
    // 2. OVER LIMIT LOGIC: Over aur ball update hone ke foran baad check karein
    if (data.innings === 2 && data.score >= data.target) {
      data.status = 'Finished';
    } else if (data.ov >= data.maxOv || data.wkts >= 10) {
      data.status = data.innings === 1 ? 'Innings Break' : 'Finished';
      ...
      {match && (
        <div style={{padding:'10px'}}>
          <div style={s.card}>
            ...
            {match.status === 'Innings Break' && <div style={s.breakBadge}>Innings Break</div>}
            {match.status === 'Finished' && <div style={s.breakBadge}>Match Finished</div>}
          </div>
...
const s = {
  ...
  breakBadge: { background: '#ef4444', color: 'white', padding: '5px 15px', borderRadius: '20px', display: 'inline-block', marginTop: '10px', fontSize: '14px', fontWeight: 'bold' },
