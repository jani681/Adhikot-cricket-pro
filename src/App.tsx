...
export default function AdhikotProUltra() {
  ...
  // Logic to handle individual Bowler and Batsman tracking
      ...
      {showFullCard && match && (
        <div style={s.overlay} onClick={() => setShowFullCard(false)}>
          <div id="full-scorecard-modal" style={{...s.modal, width:'95%', maxHeight:'90vh', overflowY:'auto'}} onClick={e => e.stopPropagation()}>
            ...
            <div style={s.flexGap}>
                <button onClick={generatePDF} style={{...s.goldBtn, flex:1}}><FaFilePdf/> DOWNLOAD PDF</button>
                <button onClick={() => setShowFullCard(false)} style={{...s.delBtn, flex:1}}>CLOSE</button>
            </div>
          </div>
        </div>
      )}
...
const s = {
  ...
  saveBtn: { padding:'15px', background:'#22c55e', color:'white', borderRadius:'10px', fontWeight:'bold', border:'none' },
