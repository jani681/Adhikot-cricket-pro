...
import { FaTrophy, FaTrash, FaSave, FaCog, FaHistory, FaEdit, FaTimes } from 'react-icons/fa';
...
export default function AdhikotProAdvanced() {
  ...
  const handleScore = (runs, type = 'normal', outType = null) => {
    ...
    if (type === 'normal') {
      ...
      if (runs === 4) { striker.fours = (striker.fours || 0) + 1; triggerAnim("FOUR! ✨"); }
      if (runs === 6) { striker.sixes = (striker.sixes || 0) + 1; triggerAnim("SIXER! 🚀"); }
    } else if (type === 'wd') {
    ...
    if (data.bl === 6) { 
        ...
        triggerAnim("OVER! 🔄"); 
        setTimeout(() => setSelModal('bwr'), 500);
    }
  ...
  const closeMatch = () => {
    if (window.confirm("Close this match and start new?")) {
    ...
    <div style={s.container}>
      ...
      {showHistory && (
        <div style={s.overlay} onClick={() => setShowHistory(false)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            <div style={s.flexBetween}><h3 style={{color:'#facc15'}}>History</h3>{isAdmin && <FaTrash onClick={() => remove(ref(db, 'matchHistory'))} color="#ef4444"/>}</div>
            <div style={{maxHeight:'60vh', overflowY:'auto'}}>
                {Object.entries(history).reverse().map(([id, m]) => (
                    <div key={id} style={s.historyItem}>
                        <b>{m.t1} vs {m.t2}</b><br/>{m.score}/{m.wkts} ({m.ov}.{m.bl})
                    </div>
                ))}
            </div>
        ...
        <div style={s.setupBox}>
          ...
          <form onSubmit={(e) => {
            ...
            <select name="batFirst" style={s.input} required>
                <option value="T1">Team A Bat First</option><option value="T2">Team B Bat First</option>
            </select>
            <textarea name="t1p" placeholder="Team A Players" style={s.area}/><textarea name="t2p" placeholder="Team B Players" style={s.area}/>
            <input name="max" placeholder="Overs" type="number" style={s.input}/><button type="submit" style={s.goldBtn}>START</button>
          </form>
        </div>
      ) : (
          ...
          <div style={s.playerCard}>
             ...
             {[match.s1, match.s2].map((p, i) => (
                <div key={i} style={match.active === (i+1) ? s.activeP : s.pRow} onClick={() => isAdmin && setSelModal(i === 0 ? 's1' : 's2')}>
                   ...
                   <span style={{flex:1, textAlign:'center'}}>{p.r || 0}({p.b || 0})</span>
                   <span style={{flex:0.5}}>{p.fours || 0}</span>
                   <span style={{flex:0.5}}>{p.sixes || 0}</span>
                   <span style={{flex:1, textAlign:'right'}}>{calculateSR(p.r, p.b)}</span>
                </div>
             ))}
             ...
             <div style={s.pRow} onClick={() => isAdmin && setSelModal('bwr')}>
                ...
                <span style={{flex:0.5}}>{match.bwr_r}</span>
                <span style={{flex:0.5, textAlign:'right'}}>{match.bwr_w}</span>
             </div>
          </div>
            ...
            <div style={s.adminGrid}>
              ...
              <div style={{gridColumn:'span 3', display:'flex', gap:'10px', marginTop:'10px'}}>
                {match.status === 'Innings Break' && (
                  <button onClick={() => update(ref(db, 'liveMatch'), {innings: 2, score: 0, wkts: 0, ov: 0, bl: 0, target: match.score + 1, status: 'Live', batTeam: match.bowlTeam, bowlTeam: match.batTeam})} style={s.saveBtn}>2ND INN</button>
                )}
                <button onClick={saveMatchToHistory} style={s.saveBtn}><FaSave/> SAVE</button>
                {match.status === 'Finished' && <button onClick={closeMatch} style={s.delBtn}><FaTimes/> CLOSE</button>}
              </div>
            </div>
          )}
        </div>
      ...
      {extraModal && (
        <div style={s.overlay}><div style={s.modal}>
          <h3>{extraModal.toUpperCase()} + Runs</h3>
          <div style={s.grid3}>
        ...
        <div style={s.overlay} onClick={() => setSelModal(null)}><div style={s.modal} onClick={e => e.stopPropagation()}>
          ...
          {(selModal === 'bwr' ? (match.batTeam === match.t1 ? match.t2p : match.t1p) : (match.batTeam === match.t1 ? match.t1p : match.t2p)).map((p, i) => (
            <div key={i} style={s.pItem} onClick={() => {
                ...
                if(selModal==='bwr') { up.bwr = p; up.bwr_r = 0; up.bwr_w = 0; up.bwr_o = 0; }
...
const s = {
  ...
  winBanner: { background:'#22c55e', color:'white', padding:'10px', borderRadius:'10px', marginBottom:'15px', fontWeight:'bold', textAlign:'center' },
  ...
  pRow: { display:'flex', justifyContent:'space-between', padding:'8px 0', fontSize:'14px' },
  ...
  historyItem: { background:'#0f172a', padding:'10px', borderRadius:'8px', marginBottom:'8px' },
  pItem: { width:'100%', padding:'12px', background:'#0f172a', color:'white', border:'none', borderBottom:'1px solid #334155', textAlign:'left' },
  bigAnim: { position:'fixed', top:'40%', left:'50%', transform:'translateX(-50%)', background:'#facc15', color:'black', padding:'10px 20px', borderRadius:'50px', fontWeight:'bold', zIndex:6000 },
